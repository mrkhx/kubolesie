...
В **личных сообщениях** сообщества бот отвечает на текст и кнопки как обычно (`Старт` / `Начать`, инвентарь, профиль).

В **групповой беседе** бот молчит, пока его не позовут. Примеры:
...
- `@kubolesie начать`
- `Помощь`
- нажатие кнопки из сообщения бота

Обычные реплики в чате («кто сегодня играет?») игнорируются. Прогресс каждого игрока считается по `from_id`, ответ уходит в ту же беседу.

Чтобы сообщество появилось в чате, в VK UI нужно разрешить добавлять сообщество в беседы и добавить его в разговор. Backend это не включает. Подробности — [docs/vk-callback-setup.md](docs/vk-callback-setup.md).
