...
import { isMineCommand, mineAct } from './mining';
import { noteRecipeCraft } from './mining-metrics';
import {
  afterCraftFlags,
...
export class GameRuntime {
    ...
    switch (command.type) {
      ...
      case 'OPEN_MENU':
        return this.openMenu(
          ctx,
          parseMenuId(String(command.payload?.menu ?? 'hub')),
          undefined,
          Number(command.payload?.page ?? 0),
        );
      case 'MINE_ACT':
        return mineAct(this.weekHost(), ctx, command.payload ?? {}, eventId);
      case 'GATHER_WOOD':
      ...
      default:
        ...
        if ((WEEK4_COMMANDS as readonly string[]).includes(command.type)) {
          return dispatchWeek4(this.weekHost(), ctx, command, eventId);
        }
  ...
  private async explore(ctx: Ctx): Promise<GameResponse> {
    ...
    if (
      loc === 'rotten_trail_edge' ||
      loc === 'split_path' ||
      loc === 'black_bark_marker'
    ) {
      return openWeek4Menu(this.weekHost(), ctx, 'trail');
    }
    if (loc === 'rot_hollow' || loc === 'sinking_ground' || loc === 'deadwood_ring') {
      return openWeek4Menu(this.weekHost(), ctx, 'hollow');
    }
    if (loc === 'missing_camp') {
      return dispatchWeek4(this.weekHost(), ctx, { type: 'WEEK4_ACT', payload: { act: 'camp' } }, 'explore-camp');
    }
    if (loc === 'warped_marker_field' || loc === 'buried_crossing' || loc === 'corrupted_node') {
      return openWeek4Menu(this.weekHost(), ctx, 'warped');
    }
    if (loc === 'rotten_seal_forecourt' || loc === 'black_root_vault' || loc === 'seal_4') {
      return openWeek4Menu(this.weekHost(), ctx, 'seal4');
    }
    if (
      loc === 'black_marsh_edge' ||
      loc === 'sunken_boardwalk' ||
      loc === 'reed_crossing'
    ) {
      return openWeek5Menu(this.weekHost(), ctx, 'marsh');
    }
    if (loc === 'black_reed_basin' || loc === 'drowned_stones' || loc === 'deep_mire') {
      return openWeek5Menu(this.weekHost(), ctx, 'basin');
    }
    if (loc === 'sunken_outpost') {
      return dispatchWeek5(this.weekHost(), ctx, { type: 'WEEK5_ACT', payload: { act: 'outpost' } }, 'explore-outpost');
    }
    if (loc === 'miremaw_lair') {
      return dispatchWeek5(this.weekHost(), ctx, { type: 'WEEK5_ACT', payload: { act: 'miremaw' } }, 'explore-miremaw');
    }
    if (loc === 'moving_marker' || loc === 'submerged_node' || loc === 'black_water_crossing') {
      return dispatchWeek5(this.weekHost(), ctx, { type: 'WEEK5_ACT', payload: { act: 'moving' } }, 'explore-moving');
    }
    if (
      loc === 'fifth_seal_approach' ||
      loc === 'sunken_seal_forecourt' ||
      loc === 'seal_3_vault'
    ) {
    ...
    if (loc === 'rem_camp' && ctx.flags.met_rem) {
      ...
      if (ctx.flags.week_4_complete) return openWeek5Menu(this.weekHost(), ctx, 'marsh');
      if (ctx.flags.week_3_complete && !ctx.flags.day_22_complete) {
        return this.renderNode(ctx.player, 'week3_complete');
      }
      if (ctx.flags.week_3_complete) return openWeek4Menu(this.weekHost(), ctx, 'trail');
  ...
  private async openInventory(ctx: Ctx): Promise<GameResponse> {
    ...
    const resourceLines = Object.entries(ctx.resources)
      ...
      .map(([key, amount]) => {
        ...
        const hint = resourceSourceHint(resource);
    ...
    const text = [
      itemLines.length ? `Инвентарь:\n${itemLines.join('\n')}` : 'Предметов нет.',
      resourceLines.length ? `Ресурсы:\n${resourceLines.join('\n')}` : '',
    ]
      .filter(Boolean)
      .join('\n');
  ...
  private async craftItem(ctx: Ctx, recipeId: string): Promise<GameResponse> {
    ...
    if (recipe.output.kind === 'resource') {
      ...
      const unlock = craftUnlockNote(recipe.id, ctx.flags);
      for (const flag of craftUnlockFlags(recipe.id)) {
      ...
      noteRecipeCraft(recipe.id);
      return this.respond(
        ctx.player,
        `Скрафчено: ${recipe.name}. +${recipe.output.amount} ${resourceLabel(recipe.output.resource)} (всего ${total}).${unlock}`,
        menu.buttons,
      );
    }
  ...
  private async startPve(
    ...
    if (isWeek4Enemy(enemyId)) {
      if (enemyId === 'tlennik') ctx.player.currentLocation = 'black_root_vault';
      else if (enemyId === 'blackroot') {
        ctx.player.currentLocation = 'deadwood_ring';
      } else if (!isWeek4Location(ctx.player.currentLocation)) {
        ctx.player.currentLocation = 'rot_hollow';
      }
      await this.store.savePlayer(ctx.player);
    }
    if (isWeek5Enemy(enemyId)) {
      if (enemyId === 'bezdonnik') ctx.player.currentLocation = 'seal_3_vault';
      else if (enemyId === 'miremaw') {
        ctx.player.currentLocation = 'miremaw_lair';
      } else if (!isWeek5Location(ctx.player.currentLocation)) {
    ...
    const week3Mods = isWeek3Enemy(enemyId)
      ...
      : null;
    const week4Mods = isWeek4Enemy(enemyId)
      ? week4Modifiers(ctx, enemyId, payload, await loadWeek4JobLevels(this.weekHost(), ctx))
      : null;
    ...
    const playerSnap: CombatantSnapshot = {
      ...
      accuracy: stats.accuracy + (mods?.player.accuracy ?? 0),
      luck: stats.luck,
    ...
    if (battle.result === 'WIN') {
      ...
      if (isWeek4Enemy(enemyId)) {
        const winNotes = await applyWeek4Victory(this.weekHost(), ctx, enemyId);
        if (winNotes.length) extra += `\n${winNotes.join(' ')}`;
        if (enemyId === 'blackroot') {
          ...
          buttons.unshift({ label: 'Низина', action: 'WEEK4_ACT', payload: { act: 'hollow' } });
        }
      }
      if (isWeek5Enemy(enemyId)) {
        const winNotes = await applyWeek5Victory(this.weekHost(), ctx, enemyId);
        if (winNotes.length) extra += `\n${winNotes.join(' ')}`;
        if (enemyId === 'miremaw') {
      ...
      if (enemyId === 'blackroot') {
        await this.store.setFlag(ctx.player.id, 'blackroot_failed', '1');
        buttons.unshift({ label: 'Ещё раз', action: 'WEEK4_ACT', payload: { act: 'blackroot' } });
      }
      if (enemyId === 'tlennik') {
        await this.store.setFlag(ctx.player.id, 'tlennik_failed', '1');
        buttons.unshift({ label: 'Подготовиться', action: 'WEEK4_ACT', payload: { act: 'prep' } });
      }
      if (enemyId === 'miremaw') {
        await this.store.setFlag(ctx.player.id, 'miremaw_failed', '1');
        buttons.unshift({ label: 'Ещё раз', action: 'WEEK5_ACT', payload: { act: 'miremaw' } });
      }
